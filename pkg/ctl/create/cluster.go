...
func createClusterCmd() *cobra.Command {
	...
	fs.StringVar(&useKopsVPC, "vpc-from-kops-cluster", "", "re-use VPC from a given kops cluster")
...
func doCreateCluster(cfg *api.ClusterConfig, name string) error {
	...
	if useKopsVPC != "" {
		...
		kw, err := kops.NewKopsWrapper(cfg, useKopsVPC)
		if err != nil {
			return err
		}
		...
		logger.Success("using VPC (%s) and subnets (%v) from kops cluster %q", cfg.VPC, cfg.Subnets, useKopsVPC)
	} else {
